...
/*!
	...
	\param szItemID The name of the branch in the hierarchy to browse. If the root branch is to be browsed then a NUL string is passed.

	\param pszContinuationPoint	If this is a secondary call to Browse,\
	...
	\param dwMaxElementsReturned	Server must not return any more elements than this value. If the server supports Continuation Points,\
	...
	\param dwBrowseFilter	An enumeration {All, Branch, Item} specifying which subset of browse elements to return.\
	...
	\param szElementNameFilter	A wildcard string that conforms to the Visual Basic LIKE operator, which will be used to filter Element names.\
												A NUL String implies no filter.

	\param szVendorFilter	A server specific filter string. This is entirely free format and may be entered by the user via an EDIT field.\
										A pointer to a NUL string indicates no filtering.

	\param bReturnAllProperties	Server must return all properties which are available for each of the returned elements.\
												If true, pdwPropertyIDs is ignored.

	\param bReturnPropertyValues	Server must return the property values in addition to the property names.

	\param dwPropertyCount	The size of the pdwPropertyIDs array.

	\param pdwPropertyIDs	An array of Property IDs to be returned with each element.
										If bReturnAllProperties is true, pdwPropertyIDs is ignored and all properties are returned.

	\param pbMoreElements		If the Server does not support a Continuation Point then the server will set pbMoreElements to True\
	...
	\param pdwCount	The size of the returned ppBrowseElements array.

	\param ppBrowseElements	Array of OPCBROWSEELEMENT returned by the Server.
*/
